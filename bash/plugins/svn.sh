...
function bjSvnGetRevision {
    revision=$(LANG=C svn info | grep "Last Changed Rev:")
    revision="r${revision##*: }"
    echo ${revision}
}
...
function bjSvnRemoveVersionInfo()
{
    ...
    printHeader "Will remove all Folders with the name .svn recursively."
    printDryRunCommand "find ${repoRoot} -name .svn -exec rm -rf {} \;"
    find ${repoRoot} -type d -name .svn -exec rm -rf {} \;
}
...
function bjListModifiedFilesInPatch()
{
    patchFile="$1"
    supressHeader="$2"
    if [ -z "${supressHeader}" ]; then
        printStatus "Files changed in the Patch: ${patchFile}"
    fi
    if [ -f ${patchFile} ]; then
        # grep -E '^(Index:|\+\+\+ )' ${patchFile} | awk '{print "bjOpenFileNB "$2}'
        grep -E '^(\+\+\+ )' ${patchFile} | awk '{print $2}' > /tmp/clipboard
        cat /tmp/clipboard
        if [ "$2" = 'dump' ]; then
            cp /tmp/clipboard ${patchFile}_filesChanged.txt
            bjSortFileEntries ${patchFile}_filesChanged.txt
        fi
    fi
    if [ -z "${supressHeader}" ]; then
        printSeparator
